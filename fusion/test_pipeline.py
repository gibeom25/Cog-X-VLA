"""
Integration test chaining blocks 6-10 end to end with synthetic tensors:
projection -> cross-attention -> FiLM -> threshold -> set-invariant encoder.
...
def run_pipeline(num_objects: int, lang_embed_dim: int = 1024, fusion_dim: int = 256,
    ...
    modules = [proj, cross_attn, film, thresh, set_enc]
    for m in modules:
        m.train(training)
    ...
    modulated = film(attended, relevance)                                      # [B, N, D]
    gated, gate = thresh(modulated, relevance)                                 # [B, N, D], [B, N]
    pooled = set_enc(gated, gate)                                              # [B, K, D]
...
if __name__ == "__main__":
    ...
    print("full Phase 2 pipeline (blocks 6->7->8->9->10) OK across all object counts and train/eval modes")
