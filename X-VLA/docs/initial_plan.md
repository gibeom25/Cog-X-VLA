...
## 1.5단계 — 로드맵 확장 (2026-08-20 추가, 착수는 1단계 완료 후)

원본 로드맵(§7)은 1단계(정지 조작 아키텍처) → 2단계(SLAM/이동) → 3단계(SSM)로 구성되어 있는데, 이번 세션에서 나온 두 제안은 이 셋 중 어디에도 속하지 않는 새로운 축(RL 기반 파지/계층적 정책)이라 **"1.5단계"로 별도 배치**한다. **1단계(본 실험, LIBERO-plus 95% 목표) 완전 검증 후에만 착수** — 지금(1일 프로토타입/1단계 구현 단계)은 설계만 기록해두고 실제 작업은 하지 않는다.

### 제안 A — Low-freq/High-freq 계층 정책 분리

- **아이디어**: 큰 이동(대략적 접근)은 지금 만든 상대좌표 기반 Low-level 정책이 낮은 주기로 담당하고, 세밀한 작업(접촉/파지)은 이미지·depth map을 직접 받는 High-level 정책이 높은 주기로 담당. 둘을 별도로 학습 후 마지막에 결합
- **평가**: 타당함. 이미 나온 결정들(wrist cam 원본 유지, 거리 기반 SAM3 재감지 주기 아이디어)과 자연스럽게 이어지는 방향 — 완전히 새로운 아이디어라기보다 그 아이디어들을 하나의 명시적 아키텍처로 통합한 것
- **리스크**: Low→High 전환 시점 결정, 두 정책의 action 결합 방식(override/residual 등) 등 계층적 제어의 고전적 난제가 그대로 발생. High-level용 RL 학습 파이프라인(reward 설계, rollout 비용)이 완전히 새로운 작업 범위

### 제안 B — 물체별 차등 표현 (장애물=반지름, 파지 대상=RL로 다듬은 pose)

- **아이디어**: 회피만 하면 되는 물체는 지금처럼 반지름(3축 반너비)으로 충분하되, 실제로 잡아야 하는 대상 물체는 대표 좌표를 쿼터니언 포함 "파지 pose"로 바꾸고, 이 접근 방향을 RL로 학습/deduction. 어떤 물체가 "파지 대상"인지는 블록7(Cross-Attention) 관련도 점수로 이미 판별 가능
- **평가**: 타당함. 기존 아키텍처(물체 토큰)의 점진적 확장에 가까워 제안 A보다 **범위가 작고 통합이 쉬움** — 새 정책을 통째로 만드는 게 아니라 필드 하나(방향)를 추가하고 그 값만 RL로 다듬는 구조
- **리스크**: 여전히 RL 학습 파이프라인 필요(reward 설계, rollout) — 다만 제안 A보다 훨씬 좁은 범위(정책 전체가 아니라 파지 방향 하나)

### 착수 시 권장 순서

1단계 완전 검증 후 이 중 하나를 고른다면 **B를 먼저** 시도할 것을 권장 — 기존 아키텍처와의 정합성이 높고 변경 범위가 작아 리스크가 낮음. A는 B의 경험(RL 파이프라인 구축 경험 포함)을 쌓은 뒤 검토.
